...
function buildMemo(data, quadrant, boostShare, baseShare, advice) {
  const now = new Date();
  const hourDealLine = Number.isFinite(extensionReferenceData.hourDealAmount)
    ? `- 小时净成交金额：${money(extensionReferenceData.hourDealAmount)}
`
    : "";
...
${hourDealLine}- 直播大屏综合 ROI：${Number.isFinite(data.overallRoi) ? data.overallRoi.toFixed(2) : "--"}
...
function storeHourReferenceData(hourCost, hourRoi) {
  const cost = extensionNumber(hourCost);
  const roi = extensionNumber(hourRoi);
  const hourDealAmount = Number.isFinite(cost) && Number.isFinite(roi) ? cost * roi : null;
  extensionReferenceData = {
    hourCost: cost,
    hourDealAmount,
  };
...
function receiveFromExtension(data) {
  ...
  if (data.pageType === "investOverview") {
    ...
    storeHourReferenceData(extractedFields.hourCost, extractedFields.hourRoi);
  }

  if (data.pageType === "liveScreen") {
    if (extractedFields.overallRoi != null) setFieldValue("overallRoi", extractedFields.overallRoi, true);
    if (extractedFields.flowSpeed != null) setFieldValue("flowSpeed", extractedFields.flowSpeed, true);
  }
...
function pollExtensionData() {
  ...
  chrome.runtime.sendMessage({ action: "getAllExtractData" }, (allData) => {
    if (chrome.runtime.lastError) return;
    if (!allData) return;
    var changed = false;
    ["extract_investOverview", "extract_liveScreen", "extract_investData", "extract_controlTable"].forEach(function(key) {
      var data = allData[key];
      if (!data?.timestamp) return;
      var tsKey = "ts_" + key;
      if (data.timestamp === (pollExtensionData._timestamps || {})[tsKey]) return;
      pollExtensionData._timestamps = pollExtensionData._timestamps || {};
      pollExtensionData._timestamps[tsKey] = data.timestamp;
      receiveFromExtension(data);
      changed = true;
    });
    if (!changed) {
      chrome.runtime.sendMessage({ action: "getExtractData" }, (data) => {
        if (chrome.runtime.lastError) return;
